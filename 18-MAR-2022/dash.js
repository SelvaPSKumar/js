    ...
    const getTable = (Datas) => {
      if (Datas.length > 0) {
        ...
        Datas.forEach(itemData => {
          ...
          template += "<td><button type='button' data-bs-toggle='modal' data-bs-target='#myModal' class='btn btn-primary btn-sm' id='editBtn' data-action='edit' data-id='" + itemData.id + "'>Edit</button><button type='button' class='btn btn-danger btn-sm ms-1' data-action='delete' data-id='" + itemData.id + "'>Delete</button></td>";
  ...
  const userForm = {
    "editNote": document.getElementById("category-type"),
    "editDateTym": document.getElementById("createdAt"),
    "editIdNum": document.getElementById("id")
}
